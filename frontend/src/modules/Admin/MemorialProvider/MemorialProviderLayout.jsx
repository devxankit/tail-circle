    ...
    <div className="flex h-screen bg-[#FAF7F2] font-sans overflow-hidden selection:bg-slate-300">
      ...
      <aside className={cn(
        ...
        <div className="h-[56px] lg:h-[72px] flex items-center px-4 lg:px-8 border-b border-neutral-800 shrink-0 overflow-hidden">
          <div className="flex items-center gap-2 cursor-pointer min-w-max" onClick={() => navigate('/vendor/memorial-provider')}>
            ...
            <span className="bg-neutral-900 border border-neutral-700 text-white text-[9px] font-bold px-2 py-0.5 rounded uppercase tracking-wider ml-1 mt-1 block sm:hidden lg:block">Memorial</span>
          </div>
        </div>
      ...
      <main className="flex-1 flex flex-col min-w-0 h-screen transition-all duration-300 sm:ml-[64px] lg:ml-[240px]">
        ...
        <header className="h-[56px] lg:h-[72px] bg-white border-b border-slate-100 flex items-center justify-between px-4 sm:px-6 lg:px-10 shrink-0 z-30 sticky top-0">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-xl font-black text-slate-900 hidden sm:block">Memorial Operations</h1>
          </div>
          ...
          <div className="flex items-center gap-3 sm:gap-6">
            ...
            {isVerified && (
              <button 
                ...
                className="hidden sm:flex items-center gap-2 bg-slate-800 hover:bg-slate-900 text-white px-4 py-2.5 rounded-full text-xs font-bold transition shadow-md hover:shadow-lg cursor-pointer"
              >
                + New Request
              </button>
            )}
              ...
              {isNotificationsOpen && isVerified && (
                <div className="absolute right-0 top-12 w-80 bg-white rounded-2xl border border-slate-100 shadow-xl overflow-hidden z-50 animate-in fade-in slide-in-from-top-4">
                  ...
                  <div className="p-3 border-t border-slate-50 text-center">
                    <button className="text-xs font-bold text-slate-500 hover:text-slate-900 transition">Mark all as read</button>
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="relative">
              <button 
                ...
                className="w-10 h-10 rounded-full bg-slate-800 text-white flex items-center justify-center font-bold text-sm shadow-md hover:shadow-lg transition cursor-pointer border-2 border-white ring-2 ring-slate-100 overflow-hidden"
              >
                {profile.logo ? <img src={profile.logo} className="w-full h-full object-cover" alt="Logo"/> : profile.businessName.charAt(0)}
              </button>
              ...
              {isProfileOpen && (
                <div className="absolute right-0 top-12 w-48 bg-white rounded-2xl border border-slate-100 shadow-xl overflow-hidden z-50 animate-in fade-in slide-in-from-top-4 py-2">
                  ...
                  <button onClick={() => navigate('/vendor/memorial-provider/settings')} className="w-full text-left px-4 py-2 text-sm font-semibold text-slate-600 hover:bg-slate-50 hover:text-slate-900 transition cursor-pointer">Account Settings</button>
