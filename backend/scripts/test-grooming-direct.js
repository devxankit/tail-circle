...
async function testGroomingDirect() {
  console.log('--- DIRECT GROOMING END-TO-END TEST ---');
  await mongoose.connect(process.env.MONGODB_URI);

  const server = http.createServer(app);
  await new Promise((resolve) => server.listen(5999, resolve));
  console.log('Test HTTP Server listening on port 5999');
  ...
  console.log('--- DIRECT GROOMING VERIFICATION SUCCESSFUL (ALL 200 OK) ---');
  process.exit(0);
}

testGroomingDirect().catch((err) => {
  console.error(err);
  process.exit(1);
});
